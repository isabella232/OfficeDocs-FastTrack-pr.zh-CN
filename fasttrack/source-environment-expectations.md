...
|Exchange Online 载入 | 您的环境必须具有以下最低级别之一：  <br/>  具有 Exchange Server 2003 前向的单个或多个 Exchange 组织。  <br/>  一个 IBM Domino 7.0.3 前向环境（ [附录 A：从 IBM Domino 迁移到 Exchange Online](from-ibm-domino-to-exchange-online.md)）。  <br/>  一个支持 Internet 邮件访问协议 (IMAP) 的电子邮件环境。  <br/>  单个 G 套件环境（仅限 Gmail、联系人和日历）。  <br/>  一个 Novell GroupWise 7.0.4 前向环境。  <br/><br/> **注意**  *若要了解多地理位置功能的信息，请参阅 [Exchange Online 中的多地理位置功能](https://go.microsoft.com/fwlink/?linkid=872776)。*           |
